...
resource "azurerm_private_dns_zone_virtual_network_link" "postgres" {
  name                  = "link-postgres-vnet-myapi-stg"
  resource_group_name   = module.environment.resource_group_name
...
module "vnet" {
  ...
  vnet_name                     = "vnet-myapi-stg"
  resource_group_name           = module.environment.resource_group_name
  location                      = module.environment.location
  address_space                 = var.vnet_address_space
  aca_subnet_cidr               = "10.1.0.0/23"
  postgres_subnet_cidr          = "10.1.4.0/24"
  private_endpoints_subnet_cidr = "10.1.5.0/24"
  runner_subnet_cidr            = "10.1.6.0/27"
  tags                          = var.tags
}
...
module "runner" {
  ...
  environment         = "stg"
  runner_labels       = "self-hosted,azure,stg"
  tags                = var.tags
}
...
module "postgres" {
  # checkov:skip=CKV_AZURE_136: B_Standard_B2ms Burstable SKU does not support geo-redundant backups
